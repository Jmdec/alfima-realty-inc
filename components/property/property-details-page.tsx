...
const UNIT_PHOTO_FIELDS: Record<string, { key: string; label: string }[]> = {
  ...
  // Regular ("property" source) agent listings — matches
  // PropertyController::UNIT_OFFER_CATEGORIES exactly
  regular_property: [
...
export default function PropertyDetailsPage({
  ...
  const unitTypeFields: { label: string; value: string; key?: string }[] =
    (() => {
      ...
      // unitTypeFields — para lang sa source === "property"
      if (source === "property") {
        return [
          {
            label: "Property Type",
            value: p.property_type
              ...
              : "—",
            // no key — not clickable, no matching upload category
          },
          {
            label: "Bedrooms",
          ...
          {
            label: "Area Min (sqm)",
            value: p.area ? String(p.area).split("-")[0]?.trim() : "—",
            key: "areaMin",
          },
          {
            label: "Area Max (sqm)",
            value: p.area?.includes("-")
              ? String(p.area).split("-")[1]?.trim()
              : "—",
            key: "areaMax",
          },
          {
  ...
  const unitPhotoFields =
    source === "property"
      ? UNIT_PHOTO_FIELDS.regular_property
  ...
  // Collage: video (or 1st image) big on the left, next 4 images in a 2x2 grid.
  // Collage: always images 1–4 in the 2x2 grid (video no longer commandeers slot 0)
  const collageImages = images.slice(1, 5);
  const remainingCount = images.length - (collageImages.length + 1);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12">
        ...
        <div className="glass rounded-xl p-7 mb-6">
          <div className="grid grid-cols-4 grid-rows-2 gap-3 h-[280px] sm:h-[360px] lg:h-[440px]">
            {/* ── Card 1: big left card — always the first image, no video overlay ── */}
            <div
