...
// ── Validation helpers ──────────────────────────────────────────────────────
function validateEmail(email: string, label: string): string {
  if (!email.trim()) return `${label} is required.`;
  const re = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  return re.test(email)
    ? ""
    : `${label} is not a valid email address (e.g. user@example.com).`;
}

function validatePhone(phone: string, label: string): string {
  if (!phone.trim()) return `${label} is required.`;
  if (!/^\d+$/.test(phone))
    return `${label} must contain digits only (no letters or symbols).`;
  if (phone.length !== 11) return `${label} must be exactly 11 digits.`;
  return "";
}
...
export default function ContactPage() {
  ...
  function runFieldValidation(name: keyof FormState, value: string): string {
    ...
    if (name === "name") return validateRequired(normalizedValue, "Name");
    if (name === "email") return validateEmail(normalizedValue, "Email");
    if (name === "phone") return validatePhone(normalizedValue, "Phone");
    if (name === "subject") return validateRequired(normalizedValue, "Subject");
    if (name === "message") return validateRequired(normalizedValue, "Message");
    return "";
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const phoneErr = validatePhone(formData.phone, "Phone");
